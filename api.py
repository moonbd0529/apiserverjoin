...
def dashboard_users():
    ...
    conn.close()
    return jsonify({
        'users': [{'user_id': u[0], 'full_name': u[1], 'username': u[2], 'join_date': u[3], 'invite_link': u[4], 'photo_url': u[5]} for u in users],
        'total': total,
        'page': page,
        'page_size': page_size
    })
            ...
            try:
                if file.mimetype.startswith('image/'):
                    ...
                    response = requests.post(
                        ...
                        data={'chat_id': user_id},
                        files={'video': open(temp_path, 'rb')}
                    )
                    save_message(user_id, 'admin', '[video]sent')
                elif file.mimetype.startswith('audio/'):
                    response = requests.post(
                        f"https://api.telegram.org/bot{config.BOT_TOKEN}/sendAudio",
                        data={'chat_id': user_id},
...
if __name__ == '__main__':
    ...
    def run_telegram_bot():
        ...
        application.add_handler(MessageHandler(filters.PHOTO, user_message_handler))
        application.add_handler(MessageHandler(filters.VIDEO, user_message_handler))
